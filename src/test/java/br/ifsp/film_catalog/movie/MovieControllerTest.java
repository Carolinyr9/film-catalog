...
class MovieControllerTest {
    ...
    void getAllMovies_shouldReturn401_whenUnauthorized() throws Exception {
        ...
        mockMvc.perform(get("/api/movies"))
                .andExpect(status().isUnauthorized()); // Spring Security handles this if not authenticated
    ...
    void getAllMovies_shouldReturn403_whenForbidden() throws Exception {
        // Based on the provided MovieController, getAllMovies does not have @PreAuthorize.
        // If a global security config was set up to deny access for certain roles, this test would apply.
        // For now, assuming no specific role is required by default, it will be 200 OK.
        // To make this test pass, Spring Security configuration would need a global rule,
        // or @PreAuthorize("hasRole('ADMIN')") would need to be added to getAllMovies.
        // As the current controller doesn't have it, this test will pass as 200 OK for a user.
        // If security config changes, this test would need to be adjusted.
        PagedResponse<MovieResponseDTO> pagedMovies = new PagedResponse<>(Collections.emptyList(), 0, 10, 0, 0, true);
        when(movieService.getAllMovies(any(Pageable.class))).thenReturn(pagedMovies);
    ...
    void getAllMovies_shouldHandlePaginationAndSorting() throws Exception {
        ...
        // Mock the service to return movies when called with specific pageable parameters
        when(movieService.getAllMovies(argThat(pageable ->
        ...
        verify(movieService).getAllMovies(argThat(pageable ->
                pageable.getPageNumber() == 1 &&
                        pageable.getPageSize() == 5 &&
                        pageable.getSort().equals(Sort.by("releaseYear").descending())
        ));
    ...
    void getMoviesByTitle_shouldReturn200AndEmptyList_whenTitleNotFound() throws Exception {
        ...
        when(movieService.getMoviesByTitle(eq("nonexistent"), any(Pageable.class))).thenReturn(emptyPagedMovies);
    ...
    void getMoviesByTitle_shouldHandlePaginationAndSorting() throws Exception {
        ...
        when(movieService.getMoviesByTitle(eq("exemplo"), argThat(pageable ->
        ...
        verify(movieService).getMoviesByTitle(eq("exemplo"), argThat(pageable ->
                pageable.getPageNumber() == 0 &&
                        pageable.getPageSize() == 5 &&
                        pageable.getSort().equals(Sort.by("releaseYear").ascending())
        ));
    ...
    void updateMovie_shouldHandleLongTextsAndSpecialCharacters() throws Exception {
        MovieRequestDTO longTextMovie = new MovieRequestDTO();
        longTextMovie.setTitle("Updated Title with Special Chars: !@#$%^&*() and a very long title " + "x".repeat(100));
        longTextMovie.setSynopsis("Updated Synopsis with Special Chars: ~`¬|\\{}[]<>,./? and a very long synopsis " + "y".repeat(100));
    ...
    void updateMovie_shouldReturn409_whenDuplicateTitleAndYear() throws Exception {
        when(movieService.updateMovie(eq(10L), any(MovieRequestDTO.class)))
                .thenThrow(new InvalidMovieStateException("Another movie with this title and year already exists"));
    ...
    void patchMovie_shouldUpdateOnlySpecifiedFields() throws Exception {
        ...
        // Prepare a response that has only these fields changed, others remain from exampleMovie
        MovieResponseDTO updatedMovie = MovieResponseDTO.builder()
        ...
        mockMvc.perform(patch("/api/movies/10")
                ...
                .andExpect(jsonPath("$.title").value(exampleMovie.getTitle())) // Verify other fields remain
    ...
    void patchMovie_shouldNotChangeOtherFields() throws Exception {
        ...
        // Simulate the service returning a movie where only synopsis is changed, others are same as exampleMovie
        MovieResponseDTO patchedMovie = MovieResponseDTO.builder()
                .id(exampleMovie.getId())
        ...
        mockMvc.perform(patch("/api/movies/10")
                ...
                .andExpect(jsonPath("$.duration").value(exampleMovie.getDuration()))
                .andExpect(jsonPath("$.contentRating").value(exampleMovie.getContentRating().toString()))
                .andExpect(jsonPath("$.genres", hasSize(1))); // Assuming genres don't change if not provided
        verify(movieService).patchMovie(eq(10L), any(MoviePatchDTO.class));
    }
